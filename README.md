...
>
> **Unit B update (2026-06-10, ADR-053 amendment).** The last sanctioned predictive test — sub-$1B **insider-cluster** event study (do stocks rise after ≥3 insiders buy within 30 days?) — is built and running on the full 2006–2024 window. A smoke run found the signal carries **real information** (+1.62% average 21-day excess return) but **fails after realistic trading costs** for thinly-traded names. Before the full run, code review caught and fixed two validity bugs: **M1** — one SEC Form 4 filed jointly by several insiders was counted as several independent buyers, fabricating "clusters" out of single decisions (detection now requires 3 distinct insiders across 3 distinct filings); **M2** — liquidity bucketing used the full 2006–2024 distribution to bin early events (a subtle look-ahead; binning is now point-in-time per event). The verdict decision tree was **pre-committed before the result exists** (PASS → 6-month paper validation → at most a 5%-of-book sleeve; INCONCLUSIVE → final kill; see `docs/superpowers/specs/2026-06-10-strategic-wrap-plan-design.md`) — the same pre-registration discipline applied to strategy, so the outcome cannot be rationalized after the fact.
>
