...
>
> **Unit B FINAL (2026-06-10, ADR-053): the seventh and last predictive test is closed.** The sub-$1B **insider-cluster** event study (do stocks rise after ≥3 insiders buy within 30 days?) ran the full 2006–2024 window on validity-amended code (**M1**: one jointly-filed Form 4 can no longer fabricate a "cluster" of insiders; **M2**: liquidity bins are now point-in-time per event). Result: **INCONCLUSIVE_THIN_COVERAGE → practical KILL.** 46.6% of the 28,866 cluster events had no usable price history in free data (delisted micro-caps over 19 years); the survivorship-honest coverage guard — which counts every unpriceable event AGAINST the test instead of quietly dropping it — fired at 24% coverage vs the required 80%. **Cannot validate ⇒ cannot ever trade.** The smoke run's promising +1.62% gross signal was a recency artifact (a 4-year window where most names still trade). The verdict branch was **pre-committed before the result existed** (see the strategic wrap spec) and executed mechanically — no goalpost moved. Prediction is now permanently closed; a definitive re-test would require survivorship-complete paid data (CRSP), outside this project's no-paid-data rail.
>
