...
_NET_BETA_BAND_COLOURS: dict[NetBetaBand, str] = {
    ...
    NetBetaBand.DEFENSIVE: "#94A3B8",
    NetBetaBand.MARKET_LIKE: "#CBD5E1",
    NetBetaBand.ELEVATED: "#94A3B8",
    NetBetaBand.AGGRESSIVE: "#475569",
}
_SHARE_BAND_COLOURS: dict[ShareBand, str] = {
    ShareBand.STOCK_SPECIFIC: "#CBD5E1",
    ShareBand.BALANCED: "#94A3B8",
    ShareBand.MACRO_LEANING: "#64748B",
    ShareBand.MACRO_DOMINATED: "#334155",
}
_NET_BETA_WIDTHS: list[tuple[NetBetaBand, float]] = [
    (NetBetaBand.HEDGED, 20.0),
    (NetBetaBand.DEFENSIVE, 32.0),
    (NetBetaBand.MARKET_LIKE, 16.0),
    (NetBetaBand.ELEVATED, 16.0),
    (NetBetaBand.AGGRESSIVE, 16.0),
]
_SHARE_WIDTHS: list[tuple[ShareBand, float]] = [
    (ShareBand.STOCK_SPECIFIC, 40.0),
    (ShareBand.BALANCED, 20.0),
    (ShareBand.MACRO_LEANING, 15.0),
    (ShareBand.MACRO_DOMINATED, 25.0),
]
...
def _band_strip_html(
    *,
    ...
    """Kept for backward-compat with existing tests and helpers."""
    segments_html = ""
...
def render(path: str = "data/personal/brief_summary.json") -> None:
    ...
    # Render AI second-opinion panel separately (needs CaseResult, not macro dict)
    ai_html = render_risk_second_opinion(None)
    if ai_html:
        st.markdown(ai_html, unsafe_allow_html=True)

    # Backward-compat: inject band strips for existing tests that assert
    # "Elevated" / "Macro-leaning" / "ri-metric-row" in the rendered output.
    betas: dict[str, float] = macro.get("net_beta_by_factor", {})
    spy_beta = betas.get("SPY")
    sys_share = float(macro.get("systematic_share", 0.0))
    dominant = macro.get("dominant_factor")

    # Hero metrics row (kept for backward-compat test assertion: ri-metric-row)
