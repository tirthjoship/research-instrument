# Phase Log — multi-modal-stock-recommender

> Historical phase-by-phase record. NOT loaded at session start. Read on demand
> only when you need the detail of a specific past phase. Current state lives in
> `docs/STATUS.md`; cross-project memory index in the .claude memory dir.
...
**Done (Discipline Calibration-Readiness Harness — SHIPPED, 2026-06-09, ADR-051):** Protects the ADR-048 discipline REDUCE-flag forward gate (the terminal bet after the 6th falsification, ADR-050) from a single-`as_of` confound found 2026-06-09 (forward log was 132 rows all dated 2026-06-08; no automated logging — documented plist ran `daily-cycle`/opportunity, never discipline). Built via subagent-driven-development (3 Sonnet implementers, Opus verification-before-completion). Pure `application/calibration_readiness.py` (stdlib, 100% cov: `as_of_spread`/`resolvable_split`/`freshness`/`readiness` READY/THIN/`diversity_label`); `discipline-calibration-status` CLI (masked, dead-cron freshness detector); **symmetric date-diversity guard** wired into `resolve-discipline-flags` → emits `INCONCLUSIVE_THIN_DATES` (blocks a confounded PROCEED AND KILL alike) until the resolved REDUCE sample spans ≥3 distinct dates over ≥10 days, THEN evaluates the **LOCKED** ADR-048 thresholds (down-rate≥0.55 AND brier≤0.45 AND n≥30, 21d). **No threshold changed** — pre-outcome design strengthening only (ADR-051 anti-p-hacking: symmetric guard, fixed K=3/D=10, fixed resolution date, no collection-extension). **Cadence (user decision):** dropped single July-15 resolve → **weekly Saturday review** (`scripts/discipline_weekly_review.sh` = holdings-risk log + resolve + status; launchd `com.tirthjoshi.stockrec.discipline-weekly`, Weekday 6) to catch drift early. No `domain/` change. **1417 tests, 93.96% cov, make check green.** Merged to develop + main (PR #31, CI green; develop≡main). Spec/plan: `docs/superpowers/{specs,plans}/2026-06-09-discipline-calibration-readiness*`. ADR-051. **Live state:** banked 2nd as_of (June 9, 21 REDUCE); log 198 rows, 2 distinct REDUCE dates, VERDICT THIN → Saturdays June 13+20 reach READY. **NEXT:** user installs the weekly launchd job (manual, local machine + gitignored holdings); each Saturday read `data/reports/discipline_weekly_review.log`; build nothing new — the gate decides the terminal shape.

**DIRECTION SET (2026-06-09, ADR-052): Deterministic Risk/CRO Engine — Alpha Hunt Closed.** Capstone decision after 6 falsifications + this session's confirmations: (1) live dogfood of `weekly-brief` on the real 66-name book — discipline side rendered real output (21 REDUCE/22 TRIM/9 HOLD/14 ADD_OK), evidence-screen side returned n=0 (abstained); also exposed a stale screen universe (delisted SIVB/PXD/SPLK/WBA/WRK + `.TO` names). (2) Independent ChatGPT + Gemini strategic reviews (honest ground-truth prompts embedding built/falsified lists to stop hallucination — Gemini's prior round invented "FinBERT", rejected ADR-004) BOTH converged with our analysis: deterministic risk/behavior is the ceiling; "your code becomes your CRO"; no credible new public-data alpha. **Decision:** product = honest CRO (risk mitigation + behavior-gap closure + don't-overtrade), recommender stays abstaining RESEARCH_ONLY screen, never predictor.
- **Execution order (NEXT SESSION starts Unit A):** **Unit A** — Portfolio Risk pillar: (1) **macro-beta scrubber** (Ridge-regress 66 holdings on TLT/UUP/USO/XLE/SPY → hidden macro bet) → (2) **cluster exposure caps** (reuse `correlation_analyzer` + `config/relationships/supply_chain.yaml` + networkx → hidden concentration) → (3) fold into `weekly-brief` + fix stale universe. **Unit B** — sub-$1B non-routine **insider-cluster IC falsification** (SEC Form-4, market-cap-tercile split, pre-registered, LOW odds/killable — last sanctioned predictive swing; KILL ⇒ prediction permanently off). **Unit C** — behavior plumbing (anti-overtrade throttle + cash-buffer policy) folded into discipline. **Background:** weekly-Saturday discipline job (`com.tirthjoshi.stockrec.discipline-weekly`, INSTALLED + smoke-tested) accrues the ADR-048/051 gate → resolves ~mid-July.
- **Caveats next session MUST know:** 65/66 accounts REGISTERED → tax-loss/wash-sale ideas MOOT (don't build); disposition-auditor both models proposed is ALREADY built (= `resolve-discipline-flags` + ADR-048 gate, don't rebuild); `domain/brief.py` already fuses 5 pillars (extend, don't rebuild); no FinBERT/LangChain/Neo4j/paid-data; flow = brainstorm→spec→plan→Sonnet→Opus-verify, LOW build / MAX verify. ADR-052.
...
**Unit B — Insider-Cluster Falsification (BUILT + REVIEWED, verdict pending, 2026-06-09/10, ADR-052→ADR-053):** The one sanctioned predictive swing. Brainstorm → spec (pre-registration, thresholds LOCKED) → plan → subagent-driven (Sonnet impl per task, Opus phase-gate + final code review). Signal = strict cluster (≥3 distinct insiders, Form-4 code-P open-market buys, 30d, fires on FILING date = point-in-time). Data = **new SEC DERA Form-345 quarterly adapter** (verified live: URL pattern, columns incl. ISSUERTRADINGSYMBOL/AFF10B5ONE, floor 2006q1) + yfinance. Gate **amended rank-IC → event-study abnormal return** (binary signal doesn't fit Spearman): Leg-1 gross `CI_low>0`, Leg-2 net-of-tiered-slippage (150/75/40bps by ADV tercile) `CI_low>0`; 3-state PASS/INCONCLUSIVE/KILL; guards THIN_N(<100)/THIN_COVERAGE(<80%). ADV-liquidity terciles replace market-cap (PIT, no shares-history leakage). Reuses `moving_block_bootstrap` (moved to `domain/bootstrap.py` to keep domain pure). **27 tests green.** Smoke (2021–24, pre-C1-fix): **INCONCLUSIVE** — gross +1.62%/21d (CI_low +0.41%) real info, net FAIL after 150bps (CI_low −1.09%); matches ADR-052 "info real, untradeable"; n=1632 bottom-tercile (well-powered). **Opus code review caught 1 Critical: C1 = coverage-guard denominator excluded delisted micro-caps (false-positive vector) — FIXED so survivorship now lowers coverage honestly; + I1 PIT guard, I2 excluded-codes footgun, I3 cache window — all fixed.** Full 2006–2024 run executing as detached background process (yfinance throttled) → verdict lands in `data/reports/insider_cluster_falsification_2024.json`. Branch `feat/insider-cluster-falsification` (18 commits, UNMERGED). ADR-053 DRAFT committed (full-window numbers `[PENDING]`). Bundled housekeeping: TSX dot→dash (GIB.A→GIB-A.TO) + ANSS prune. **NEXT: read verdict JSON → fill ADR-053 → STATUS/PHASE_LOG → finish branch.**

## 2026-06-10 — Strategic wrap locked; Unit B amended + rerun; dashboard designed
- /code-review found C1 (CLI echo KeyError), M1 (joint-filing cluster fabrication),
  M2 (pooled/colliding terciles). All fixed via Sonnet implementers + Opus phase gates.
- Strategic wrap spec committed: pre-committed Unit B verdict tree, 5% sleeve cap,
  honest self-sustainability (deterministic + fail-loud), close ~Jun 29.
- Full 2006-2024 run relaunched on amended code (nohup, survives sessions).
