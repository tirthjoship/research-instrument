...
from domain.services import (
    classify_horizon, grade_from_horizons, validate_data_freshness,
    validate_feature_matrix, validate_point_in_time_access,
)
...
def test_grade_strong_buy() -> None:
    pred = MultiHorizonPrediction(
        ...
        confidence_5d=0.7, confidence_10d=0.7,
    )
...
def test_grade_buy_one_horizon_bullish() -> None:
    pred = MultiHorizonPrediction(
        predicted_return_2d=0.025, predicted_return_5d=0.005,
        predicted_return_10d=0.01, confidence_2d=0.7,
        confidence_5d=0.5, confidence_10d=0.5,
    )
    grade, signals = grade_from_horizons(pred)
...
def test_grade_buy_two_horizons_low_magnitude() -> None:
    pred = MultiHorizonPrediction(
        predicted_return_2d=0.02, predicted_return_5d=0.03,
        predicted_return_10d=0.04, confidence_2d=0.7,
        confidence_5d=0.7, confidence_10d=0.7,
    )
...
def test_grade_hold_all_neutral() -> None:
    pred = MultiHorizonPrediction(
        predicted_return_2d=0.005, predicted_return_5d=0.01,
        predicted_return_10d=0.02, confidence_2d=0.5,
        confidence_5d=0.5, confidence_10d=0.5,
    )
    grade, signals = grade_from_horizons(pred)
...
def test_grade_hold_conflicting_signals() -> None:
    pred = MultiHorizonPrediction(
        predicted_return_2d=0.03, predicted_return_5d=-0.04,
        predicted_return_10d=0.01, confidence_2d=0.7,
        confidence_5d=0.7, confidence_10d=0.5,
    )
    grade, signals = grade_from_horizons(pred)
    assert grade == RecommendationGrade.HOLD
    assert signals["2d"] == "bullish"
...
def test_grade_may_sell() -> None:
    pred = MultiHorizonPrediction(
        predicted_return_2d=0.005, predicted_return_5d=-0.025,
        predicted_return_10d=0.01, confidence_2d=0.5,
        confidence_5d=0.7, confidence_10d=0.5,
    )
    grade, _ = grade_from_horizons(pred)
...
def test_grade_immediate_sell() -> None:
    pred = MultiHorizonPrediction(
        predicted_return_2d=-0.025, predicted_return_5d=-0.04,
        predicted_return_10d=-0.06, confidence_2d=0.8,
        confidence_5d=0.8, confidence_10d=0.8,
    )
    grade, signals = grade_from_horizons(pred)
...
def test_validate_feature_matrix_all_leakage_columns() -> None:
    for col in ["next_day_return", "next_week_return", "future_earnings_surprise", "forward_pe_ratio"]:
        with pytest.raises(LookAheadBiasError):
...
def test_validate_point_in_time_pass() -> None:
    pt = datetime.now()
    signals = [Signal(symbol="AAPL", timestamp=pt - timedelta(days=1), price=100.0, volume=1000.0, open_=99.0, high=101.0, low=98.0)]
    sentiments = [Sentiment(source="news", timestamp=pt - timedelta(hours=1), sentiment_score=0.0, confidence=1.0)]
